...
func _start_mission() -> void:
	...
	var walker_wave := WaveEntry.new()
	walker_wave.enemy_type = walker_enemy_type
	walker_wave.count = 3
	walker_wave.spawn_interval = 1.0
	walker_wave.start_delay = 0.5
	...
	mission.waves = [runner_wave, walker_wave]
...
func _on_zombie_spawned(spawned_zombie: Zombie) -> void:
	print("Spawned zombie instance: ", spawned_zombie.get_instance_id())
	...
	spawned_zombie.died.connect(func(dead_zombie: Zombie) -> void: 
		typing_controller.unregister_target(dead_zombie), CONNECT_ONE_SHOT
	)
